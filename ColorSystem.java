package sortingvisualizer_honor;

import javafx.scene.paint.Color;
import javafx.scene.paint.Paint;

public interface ColorSystem {
    public static final Paint TEAGREEN = Color.web("#d0f0c0");
    public static final Paint DARTHMOUNTHGREEN = Color.web("#00693e");
    public static final Paint MALACHITEGREEN = Color.web("#0bda51");
    public static final Paint HARLEQUINGREEN = Color.web("#3fff00");
    public static final Paint SPRINGGREEN = Color.web("#00ff7f");
}
